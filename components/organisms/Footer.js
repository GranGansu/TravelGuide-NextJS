    ...
    <footer className='w-full  bg-gray-900 pb-10 flex flex-col items-center  gap-x-10 relative'>
      ...
      <div className='flex'>
        <div className='flex flex-col py-12  text-white w-full sm:w-fit px-4 mx-auto relative z-10'>
          <h2 className='text-xl text-white p-4 mb-2 bg-transparent bg-gradient-to-r rounded-xl border-2 border-blue-500 flex gap-x-2'>
          ...
          <div className='flex flex-col gap-2 overflow-x-auto overflow-y-visible'>
            {secciones.map((tt, key) => {
              return (
                <Link key={key} href={'/' + tt.url}>
                  <div className='p-2 text-xl font-bold hover:bg-gray-600 hover:cursor-pointer'>{t(tt.title)}</div>
                </Link>
              );
            })}
          </div>
        ...
        <div className='flex flex-col py-12  text-white w-full sm:w-fit px-4 mx-auto relative z-10'>
          <h2 className='text-xl text-white p-4 mb-4 bg-transparent bg-gradient-to-r rounded-xl border-2 border-blue-500 flex gap-x-2'>
            {/*             <svg className='fill-white' width='24' height='24' xmlns='http://www.w3.org/2000/svg' fillRule='evenodd' clipRule='evenodd'>
              <path d='M13 9h9l-14 15 3-9h-9l14-15-3 9zm-8.699 5h8.086l-1.987 5.963 9.299-9.963h-8.086l1.987-5.963-9.299 9.963z' />
            </svg> */}
            Redes sociales
          </h2>
          <ul className='flex flex-col gap-6 items-center text-xl font-bold relative z-10'>
            {redes.map((r) => {
              return (
                <li key={r.name} className='border-white text-xl px-4 rounded hover:cursor-pointer bg-gray-900/70'>
                  {r.name}
                  {r.follow && <span className='border-red-500 border-2 text-sm py-1 ml-2 px-2 rounded-full'>{r.follow}</span>}
                </li>
              );
            })}
          </ul>
      ...
      <div className=' hidden flex text-white pt-8 mx-4 w-full justify-center items-center relative mb-10'>
        <ul className='flex flex-col gap-6 items-center text-xl font-bold relative z-10'>
          {redes.map((r) => {
            return (
              <li key={r.name} className='border-white text-xl px-4 rounded hover:cursor-pointer bg-gray-900/70'>
                {r.name}
                {r.follow && <span className='bg-red-500 py-1 ml-2 px-2 rounded-full'>{r.follow}</span>}
              </li>
            );
          })}
        </ul>
      </div>
      <p className='text-gray-400'>See&Do Copyright 2023</p>
    </footer>
  );
}
