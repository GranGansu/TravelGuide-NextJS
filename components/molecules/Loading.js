...
export default function Loading({ className }) {
  ...
  return (
    <div className={`${className} fill-white flex flex-col items-center gap-y-2`}>
      <motion.svg
        initial={{ rotate: 0 }}
      ...
      <motion.p transition={{ duration: 0.9, repeat: Infinity, repeatType: 'mirror' }} animate={{ opacity: 1 }} initial={{ opacity: 0.4 }}>
        {t('loading')}
      </motion.p>
    </div>
  );
